...
function getlist() {
  global $data_dir, $id_file, $tags_file, $kindle_file, $paper_file, $paper_notify_email_file;

  $list = glob($data_dir.'*.json', GLOB_NOSORT);
  if ($list) {
    $list = array_diff($list, array($id_file, $tags_file, $kindle_file, $paper_file, $paper_notify_email_file));
    natsort($list);
